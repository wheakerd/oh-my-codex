...
import { mintStateAuthorityTransportCapability,
	resolveStateAuthorityForGuard,
} from "../../state/authority.js";
import { writeSessionStart } from "../../hooks/session.js";

import {describe, it } from 'node:test';
import { runHudAuthorityTick as runHudAuthorityTickRaw} from '../authority.js';

const fixtureAuthority = new Map<string, Promise<void>>() ;
let fixtureSessionSequence = 0;

async function prepareHudAuthority(cwd: string): Promise<void> {
  let prepared = fixtureAuthority.get(cwd);
	if (!prepared) {
		prepared = ( async () => {
    for (const directory of [ cwd, join(cwd, '.omx'), join(cwd, '.omx', 'state'), join(cwd, '.omx', 'state', "authority"), ]) {
				if (existsSync(directory)) await chmod(directory, 0o700);
          }const sessionId = `hud-authority-${++fixtureSessionSequence
          }`;
		...
		fixtureAuthority.set(cwd, prepared);
	}
	await prepared;
}

async function runHudAuthorityTick(
	options: Parameters<typeof runHudAuthorityTickRaw>[0],
	deps: Parameters<typeof runHudAuthorityTickRaw>[1] = {},
): Promise<void> {
	if (existsSync(options.cwd)) await prepareHudAuthority(options.cwd);
	return runHudAuthorityTickRaw(options, deps);
}
...
describe("runHudAuthorityTick", () => {
	it("writes a live HUD authority owner lease before ticking", async () => {
		...
		try {
			...
			const lease = JSON.parse(
				await readFile(
					join(cwd, ".omx", "state", "notify-fallback-authority-owner.json"),
					"utf-8",
				),
			);
			...
			assert.ok(
				typeof lease.heartbeat_at === "string" && lease.heartbeat_at.length > 0,
			);
		} finally {
	...
	it("keeps HUD authority writes and fallback watcher launches bound to the committed parent when the pane cwd drifts into a nested project", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "omx-hud-authority-parent-"));
		const nestedCwd = join(cwd, "nested-project");
		const calls: Array<{
			args: string[];
		...
		try {
			...
			await prepareHudAuthority(cwd);
			await runHudAuthorityTickRaw(
				{
					cwd,
					frameCwd: nestedCwd,
					nodePath: "/node",
				...
				{
					runProcess: async (_nodePath, args, options) => {
			...
			const lease = JSON.parse(
				await readFile(
					join(stateDir, "notify-fallback-authority-owner.json"),
					"utf-8",
				),
			);
			assert.equal(
				lease.cwd,
				nestedCwd,
				"the live pane cwd is retained only as diagnostic metadata",
			);
			assert.equal(
				existsSync(join(nestedCwd, ".omx")),
				false,
				"a nested pane cwd must not materialize authority state",
			);
			assert.equal(calls.length, 1);
			assert.equal(calls[0]!.args[4], cwd);
			assert.equal(calls[0]!.options.cwd, cwd);
			assert.equal(calls[0]!.options.env.OMX_TEAM_STATE_ROOT, stateDir);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	});

	it("suppresses HUD authority mutation and watcher spawning without committed authority", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "omx-hud-authority-missing-"));
		try {
			let spawned = false;
			await assert.rejects(
				runHudAuthorityTickRaw(
					{ cwd, nodePath: "/node", packageRoot: "/pkg" },
					{
						runProcess: async () => {
							spawned = true;
				...
				/committed active authority|state authority/i,
			);
	...
	it("does not materialize authority state under a deleted cwd marker path", async () => {
		const parent = mkdtempSync(
			join(tmpdir(), "omx-hud-authority-deleted-marker-"),
		);
	...
	it("writes authority state under a real existing cwd that literally ends with the marker text", async () => {
		const parent = mkdtempSync(
			join(tmpdir(), "omx-hud-authority-live-marker-"),
		);
	...
	it("invokes fallback watcher in authority-only mode with HUD env", async () => {
		...
		try {
			...
			assert.equal(
				call.args[0].endsWith("/dist/scripts/notify-fallback-watcher.js"),
				true,
			);
	...
	it("falls back to CLI entry dist scripts when package root lacks dist scripts", async () => {
		const packageRoot = await mkdtemp(
			join(tmpdir(), "omx-hud-package-root-no-dist-"),
		);
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hud-authority-entry-fallback-"),
		);
	...
	it("rate-limits repeated HUD authority watcher spawns and records diagnostics", async () => {
		...
		try {
			...
			assert.equal(
				calls.length,
				1,
				"second HUD frame should not respawn an authority-only child",
			);
			const state = JSON.parse(
				await readFile(
					join(cwd, ".omx", "state", "notify-fallback-authority-state.json"),
					"utf-8",
				),
			);
	...
	it("allows HUD authority watcher spawn after the rate-limit window elapses", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hud-authority-rate-limit-elapsed-"),
		);
		let calls = 0;
		try {
	...
	it("preserves cooldown when a contender observes the rate limit only after taking the lock", async () => {
		...
		try {
			...
			await writeFile(
				statePath,
				JSON.stringify(
					{ ...spawnedState, next_allowed_at: new Date(0).toISOString() },
					null,
					2,
				),
			);
			...
			assert.equal(
				calls,
				1,
				"contender should re-check cooldown after locking and skip spawning",
			);
	...
	it("does not release a lock that was replaced by a newer owner", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hud-authority-token-release-"),
		);
		const lockPath = join(
			cwd,
			".omx",
		...
		try {
				...
				},
				{
					...
					onLockAcquired: async () => {
						...
						await writeFile(
							...
							JSON.stringify({ token: "newer-owner" }, null, 2),
						);
					},
					runProcess: async () => {
						calls += 1;
			...
			assert.equal(
				existsSync(lockPath),
				true,
				"stale releaser must not remove a newer lock owner",
			);
	...
	it("reaps a stale authority lock before acquiring a fresh lock", async () => {
		...
		try {
			...
			await writeFile(
				join(lockPath, "owner.json"),
				JSON.stringify({ token: "stale-owner" }, null, 2),
			);
			...
			assert.equal(
				existsSync(lockPath),
				false,
				"fresh authority lock should be released after successful spawn",
			);
			const state = JSON.parse(
				await readFile(
					join(cwd, ".omx", "state", "notify-fallback-authority-state.json"),
	...
	it("fails closed before spawning when the rate-limit state cannot be persisted", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hud-authority-persist-failure-"),
		);
		let calls = 0;
		try {
			await runHudAuthorityTick(
			...
			assert.equal(
				calls,
				0,
				"authority child should not spawn without durable rate-limit state",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("fails closed without spawning when the authority state is unreadable", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hud-authority-invalid-state-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const statePath = join(stateDir, "notify-fallback-authority-state.json");
		let calls = 0;
		try {
			...
			assert.equal(
				calls,
				0,
				"authority child should not spawn when cooldown state cannot be trusted",
			);
			const state = JSON.parse(await readFile(statePath, "utf-8"));
			...
			assert.equal(
				calls,
				0,
				"repaired invalid-state diagnostic should still enforce cooldown on the next tick",
			);
	...
	it("fails closed without spawning when the authority state has an invalid shape", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-hud-authority-invalid-shape-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const statePath = join(stateDir, "notify-fallback-authority-state.json");
		let calls = 0;
		try {
			...
			assert.equal(
				calls,
				0,
				"authority child should not spawn when parsed state shape cannot be trusted",
			);
			const state = JSON.parse(await readFile(statePath, "utf-8"));
			assert.equal(state.last_status, "failed");
			assert.equal(state.last_reason, "invalid_authority_state");
			...
			await writeFile(
				statePath,
				JSON.stringify({ ...state, next_allowed_at: "not-a-date" }),
			);
			...
			assert.equal(
				calls,
				0,
				"authority child should not spawn when next_allowed_at is malformed",
			);
			...
			assert.equal(
				repairedState.next_allowed_at,
				new Date(7_000).toISOString(),
			);
			assert.match(
				repairedState.last_error,
				/next_allowed_at must be a valid ISO timestamp/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("surfaces authority child stderr on failure", async () => {
		...
		try {
			...
			writeFileSync(
				failingScript,
				"console.error('notify script missing during rebuild'); process.exit(1);\n",
			);
	...
	it("surfaces nonzero authority child status when the child is silent", async () => {
		const cwd = mkdtempSync(
			join(tmpdir(), "omx-hud-authority-silent-failure-"),
		);
		try {
			mkdirSync(join(cwd, "dist", "scripts"), { recursive: true });
			const failingScript = join(
