...
async function initializeNotifyFixtureAuthority(
	...
	const previousAuthorityEnv = new Map(
		[
			...TEAM_AMBIENT_STATE_ROOT_ALIAS_ENV_KEYS,
			...TEAM_STATE_AUTHORITY_TRANSPORT_ENV_KEYS,
			"OMX_SESSION_ID",
	...
	for (const key of previousAuthorityEnv.keys()) delete process.env[key];
	try {
		...
		await initializeStateAuthority({
			startup_cwd: cwd,
			observed_cwd: cwd,
			launch_id: `notify-regression-205-${sessionId}-${Date.now()}`,
		...
		for (const [key, value] of previousAuthorityEnv) {
...
async function withTempWorkingDir(
	...
	const sessionId = "sess-managed-regression";
	await initializeNotifyFixtureAuthority(cwd, sessionId);
	const authorityEnv = fixtureAuthorityEnv.get(cwd) ?? {};
	const fixtureEnvKeys = new Set([
		...Object.keys(authorityEnv),
		...TEAM_AMBIENT_STATE_ROOT_ALIAS_ENV_KEYS,
		...TEAM_STATE_AUTHORITY_TRANSPORT_ENV_KEYS,
	]);
	const previous = new Map(
		[...fixtureEnvKeys].map((key) => [key, process.env[key]]),
	);
	for (const key of TEAM_AMBIENT_STATE_ROOT_ALIAS_ENV_KEYS)
...
describe("regression-205: DEFAULT_STALL_PATTERNS excludes permission-seeking prompts after #1416", () => {
	it('DEFAULT_STALL_PATTERNS array does not include "if you want"', async () => {
		const { DEFAULT_STALL_PATTERNS } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		assert.ok(
...
describe('regression-205: detectStallPattern excludes "if you want" after #1416', () => {
	it('does not detect "if you want" pattern', async () => {
		const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule(
			"notify-hook/auto-nudge.js",
		);
		assert.equal(
			detectStallPattern(
				"If you want, I can refactor the module.",
		...
		await withTempWorkingDir(async (cwd) => {
			...
			const result = runNotifyHook(cwd, fakeBinDir, codexHome, {
				"last-assistant-message":
					"I checked the files. If you want, I can keep going and apply the fix.",
			});
			assert.equal(
				result.status,
				0,
...
describe("regression-205: resolveTeamStateDirForWorker is exported from team-worker.js", () => {
	...
	it("fails closed for OMX_TEAM_STATE_ROOT when worker identity is missing", async () => {
		const { resolveTeamStateDirForWorker } = await loadModule(
			"notify-hook/team-worker.js",
		);
		const saved = process.env.OMX_TEAM_STATE_ROOT;
		process.env.OMX_TEAM_STATE_ROOT = "/custom/state/root";
		try {
			const result = await resolveTeamStateDirForWorker("/some/cwd", {
				teamName: "fix-ts",
				workerName: "worker-1",
			});
	...
	it("does not guess {cwd}/.omx/state when no worker identity exists", async () => {
		...
		try {
			...
			const result = await resolveTeamStateDirForWorker(cwd, {
				teamName: "fix-ts",
				workerName: "worker-1",
			});
			assert.equal(result, null);
