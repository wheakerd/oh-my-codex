...
import {
	initTeamState as initTeamStateWithAuthority,
	enqueueDispatchRequest as enqueueDispatchRequestWithAuthority,
	readDispatchRequest,
} from "../../team/state.js";
import {
...
import { writeSessionStart as writeSessionStartWithAuthority } from "../session.js";
import {
...
const SESSION_SCOPED_WATCHER_STATE_FILES = new Set([
...
function canonicalWatcherFixtureStatePath(path: string): string {
	if (!path) return path;
	for (const [cwd, sessionId] of watcherFixtureSessionIds) {
		const rootStatePrefix = join(cwd, ".omx", "state") + "/";
		if (!path.startsWith(rootStatePrefix)) continue;
		const filename = path.slice(rootStatePrefix.length);
		if (filename.startsWith("team/"))
			return join(cwd, ".omx", "state", "sessions", sessionId, filename);
		if (!SESSION_SCOPED_WATCHER_STATE_FILES.has(filename)) return path;
		return join(cwd, ".omx", "state", "sessions", sessionId, filename);
	}
	return path;
}
...
async function mkdir(
	path: string,
	options?: Parameters<typeof mkdirRaw>[1],
): Promise<string | undefined> {
	const stateMarker = "/.omx/state/";
	const stateMarkerIndex = path.indexOf(stateMarker);
	if (stateMarkerIndex > 0) {
		const cwd = path.slice(0, stateMarkerIndex);
		const relativeStatePath = path.slice(stateMarkerIndex + stateMarker.length);
		const sessionMatch = /^sessions\/([^/]+)(?:\/|$)/.exec(relativeStatePath);
		await installWatcherFixtureAuthority(cwd, sessionMatch?.[1]);
	}
	return mkdirRaw(canonicalWatcherFixtureStatePath(path), options);
}

async function writeFile(
	path: string,
	data: Parameters<typeof writeFileRaw>[1],
	options?: Parameters<typeof writeFileRaw>[2],
): Promise<void> {
	const stateMarker = "/.omx/state/";
	const stateMarkerIndex = path.indexOf(stateMarker);
	if (stateMarkerIndex > 0) {
	...
	await writeFileRaw(canonicalWatcherFixtureStatePath(path), data, options);
}

async function readFile(
	path: string | URL,
	options: Parameters<typeof readFileRaw>[1] = "utf-8",
): Promise<string> {
	const value = await readFileRaw(
		typeof path === "string" ? canonicalWatcherFixtureStatePath(path) : path,
		options,
	);
	return typeof value === "string" ? value : value.toString("utf-8");
}

async function installWatcherFixtureAuthority(
	cwd: string,
	sessionId?: string,
): Promise<void> {
	if (watcherFixtureSessionIds.has(cwd)) return;
	for (const key of TEAM_STATE_AUTHORITY_TRANSPORT_ENV_KEYS)
		delete process.env[key];
	delete process.env.OMX_SESSION_ID;
	const effectiveSessionId = sessionId ?? "notify-fallback-control";
	watcherFixtureSessionIds.set(cwd, effectiveSessionId);
	await mkdir(join(cwd, ".omx", "state"), { recursive: true, mode: 0o700 });
	hardenTestAuthorityTreeSync(cwd);
	await mkdir(join(cwd, ".omx", "state", "sessions", effectiveSessionId), {
		recursive: true,
		mode: 0o700,
	});
	hardenTestAuthorityTreeSync(cwd);
	await initializeStateAuthority({
		startup_cwd: cwd,
		observed_cwd: cwd,
		launch_id: `notify-fallback-${effectiveSessionId}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		session_binding: { canonical_session_id: effectiveSessionId },
	});
	await writeSessionStartWithAuthority(cwd, effectiveSessionId);
	...
	const transport = buildStateAuthorityTransportEnv(refreshedAuthority, {
		OMX_SESSION_ID: effectiveSessionId,
	});
	hardenTestAuthorityTreeSync(cwd);
	watcherFixtureAuthorityEnvs.set(cwd, transport);

	Object.assign(process.env, transport);
}

async function writeSessionStart(
	cwd: string,
	sessionId: string,
): Promise<void> {
	await installWatcherFixtureAuthority(cwd, sessionId);
	hardenTestAuthorityTreeSync(cwd);
}

async function initTeamState(
	...args: Parameters<typeof initTeamStateWithAuthority>
) {
	const cwd = args[4] ?? process.cwd();
	await installWatcherFixtureAuthority(cwd);
...
async function enqueueDispatchRequest(
	...args: Parameters<typeof enqueueDispatchRequestWithAuthority>
) {
	const cwd = args[2] ?? process.cwd();
	await installWatcherFixtureAuthority(cwd);
	const result = await enqueueDispatchRequestWithAuthority(...args);
	hardenTestAuthorityTreeSync(cwd);
	return result;
}
...
const inheritedAuthorityTransport = new Map<string, string | undefined>();
...
before(() => {
	...
	for (const key of TEAM_STATE_AUTHORITY_TRANSPORT_ENV_KEYS) {
		inheritedAuthorityTransport.set(key, process.env[key]);
		delete process.env[key];
	}
});
...
after(() => {
	...
	for (const [key, value] of inheritedAuthorityTransport) {
		if (typeof value === "string") process.env[key] = value;
		else delete process.env[key];
	}
	watcherFixtureAuthorityEnvs.clear();
	watcherFixtureSessionIds.clear();
});
...
async function writeCanonicalWatcherTeamFixture(
	...
	await installWatcherFixtureAuthority(wd, sessionId);
	const stateDir = join(wd, ".omx", "state", "sessions", sessionId);
	const teamDir = join(stateDir, "team", teamName);
	const nowIso = new Date().toISOString();

	await mkdir(join(wd, ".omx", "logs"), { recursive: true, mode: 0o700 });

	hardenTestAuthorityTreeSync(wd);
	await mkdir(join(teamDir, "workers"), { recursive: true, mode: 0o700 });
	hardenTestAuthorityTreeSync(wd);
	await writeFile(
	...
	const manifest = {
		...
		worker_count: 1,
		next_task_id: 1,
		workers: [{ name: "worker-1", index: 1, pane_id: "%42", role: "executor" }],
		created_at: nowIso,
	};
	await writeFile(
		...
		JSON.stringify(
			{
				...
				tmux_session: `${teamName}:0`,
				leader_pane_id: "%42",
				workers: [{ name: "worker-1", pane_id: "%42" }],
			},
			null,
			2,
		),
		...
		JSON.stringify(
			{
				...
				transitions: terminal
					? [{ from: "team-exec", to: "complete", at: nowIso }]
					: [],
			},
			null,
			2,
...
function defaultAutoNudgePattern(targetPane: string): RegExp {
	return new RegExp(
		`send-keys -t ${targetPane} -l ${DEFAULT_AUTO_NUDGE_RESPONSE.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&")} \\[OMX_TMUX_INJECT\\]`,
	);
}
...
function buildManagedRalphTmux(
	...
	const panes =
		codexPanes && codexPanes.length > 0
			? codexPanes
			: [
					{
						paneId: livePane,
...
function buildWatcherFixtureEnv(
	...
	return {
		...buildCleanNotifyEnv(),
		...watcherFixtureAuthorityEnvs.get(cwd),
		...overrides,
	};
}
...
describe("notify-fallback watcher", () => {
	it("uses offset-bounded rollout reads instead of re-reading whole tracked files", async () => {
		const source = await readFile(
			new URL("../../scripts/notify-fallback-watcher.js", import.meta.url),
			"utf-8",
		);
		...
		assert.match(
			source,
			/const fileStat = await stat\(path\)\.catch\(\(\) => null\);\s*if \(!fileStat\)\s*continue;/,
		);
		assert.match(
			source,
	...
	it("one-shot mode forwards only recent task_complete events", async () => {
		...
		const rolloutPath = join(
			sessionDir,
			`rollout-test-fallback-once-${sid}.jsonl`,
		);

		try {
			await installWatcherFixtureAuthority(wd, sid);
			...
			hardenTestAuthorityTreeSync(wd);
			hardenTestAuthorityTreeSync(wd);
			const stateDir = join(wd, ".omx", "state", "sessions", sid);
			...
			await writeFile(
				rolloutPath,
				`${lines.map((v) => JSON.stringify(v)).join("\n")}\n`,
			);
			hardenTestAuthorityTreeSync(wd);
			...
			assert.deepEqual(
				fallbackEntries.map((entry) => entry.type),
				["fallback_notify"],
			);
			...
			assert.equal(
				completedThread?.completion_source,
	...
	it("rotates notify-fallback logs when the size cap is exceeded", async () => {
		...
		const rolloutPath = join(
			sessionDir,
			`rollout-test-fallback-rotate-${sid}.jsonl`,
		);
		...
		try {
			...
			const threadId = `thread-${sid}`;
			const turnIds = ["first", "second", "third"].map(
				(label) => `turn-${label}-${sid}`,
			);
			...
			assert.deepEqual(
				currentEntries.map((entry) => entry.type),
				["fallback_notify"],
			);
			assert.deepEqual(
				rotatedEntries.map((entry) => entry.type),
				["fallback_notify"],
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("streaming mode buffers partial JSON lines until the newline arrives", async () => {
		...
		const rolloutPath = join(
			sessionDir,
			`rollout-test-fallback-stream-partial-${sid}.jsonl`,
		);
...
`,
			...
			await writeFile(
				rolloutPath,
				`${await readFile(rolloutPath, "utf-8")}${partialPrefix.slice(0, splitAt)}`,
			);
			...
			assert.equal(
				beforeLines.length,
				0,
				"partial line should not be emitted before newline completes it",
			);

			await writeFile(
				rolloutPath,
				`${await readFile(rolloutPath, "utf-8")}${partialPrefix.slice(splitAt)}\n`,
			);
			...
			await waitFor(
				async () => {
					...
					return (
						turnLines.length === 1 &&
						new RegExp(partialTurn).test(turnLines[0] ?? "")
					);
				},
				4000,
				75,
	...
	it("streaming mode preserves multibyte text split across polling reads", async () => {
		...
		try {
			...
			assert.ok(
				emojiOffset > 0,
				"expected test payload to contain emoji bytes",
			);

			await appendFile(rolloutPath, bytes.subarray(0, emojiOffset + 1));
			await sleep(250);
			assert.equal(
				(await readLines(turnLog)).length,
				0,
			...
			assert.equal(
				(await readLines(turnLog)).length,
				0,
				"transient missing file should preserve buffered bytes",
			);
			...
			await waitFor(
				async () => {
					...
					return (
						turnLines.length === 1 &&
						turnLines[0]!.includes(utf8Turn) &&
						turnLines[0]!.includes(emojiMessage)
					);
				},
				4000,
				75,
		...
		try {
			...
			await waitFor(
				async () => {
					...
					return (
						turnLines.length === 1 &&
						new RegExp(newTurn).test(turnLines[0] ?? "")
					);
				},
				4000,
				75,
	...
	it("records explicit leader-only dispatch drain state in test mode", async () => {
		...
		try {
			...
			const watcherState = JSON.parse(
				await readFile(watcherStatePath, "utf-8"),
			);
	...
	it("suppresses authority-only control-plane ticks when only skill-active-state carries the deep-interview input lock", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-authority-skill-lock-"),
		);
		try {
			hardenTestAuthorityTreeSync(wd);
			await mkdir(join(wd, ".omx", "logs"), { recursive: true });
	...
	it("backs off authority-only nudge ticks when the primary watcher is healthy", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-authority-backed-off-"),
		);
		...
		try {
			...
			await mkdir(
				join(wd, ".omx", "state", "sessions", "sess-managed-fallback"),
				{ recursive: true },
			);
			hardenTestAuthorityTreeSync(wd);
			await writeFile(
			...
			await writeFile(
				join(wd, ".omx", "state", "notify-fallback.pid"),
				JSON.stringify(
					{
						pid: process.pid,
						cwd: wd,
			...
			await writeFile(
				join(wd, ".omx", "state", "notify-fallback-state.json"),
				JSON.stringify(
					{
						pid: process.pid,
						cwd: wd,
	...
	it("disables fallback watcher nudges when deep-interview state is active", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-deep-interview-suppressed-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		try {
			...
			await mkdir(join(wd, ".omx", "logs"), { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			await mkdir(join(wd, ".omx", "state", "team", "dispatch-team"), {
				recursive: true,
			});
			hardenTestAuthorityTreeSync(wd);
			await mkdir(fakeBinDir, { recursive: true });
			...
			await writeFile(
				join(wd, ".omx", "state", "ralph-state.json"),
				JSON.stringify(
					{
						active: true,
			...
			await writeFile(
				join(wd, ".omx", "state", "hud-state.json"),
				JSON.stringify(
					{
						last_turn_at: new Date(Date.now() - 300_000).toISOString(),
						turn_count: 3,
			...
			await writeFile(
				join(wd, ".omx", "state", "team", "dispatch-team", "config.json"),
				JSON.stringify(
					{
						name: "dispatch-team",
	...
	it("disables fallback watcher nudges when only skill-active-state carries the deep-interview input lock", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-deep-interview-skill-lock-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		try {
	...
	it("runs leader nudge checks from the fallback watcher so stale alerts do not wait for a leader turn", async () => {
		...
		const tmuxLogPath = join(wd, "tmux.log");
		try {
			await installWatcherFixtureAuthority(wd, "sess-canonical-inactive");
			...
			assert.match(
				tmuxLog,
				/send-keys -t %42 -l Team dispatch-team: leader stale, \d+ worker pane\(s\) still active\./,
			);
			...
			const nudgeEvent = logEntries.find(
				(entry: { type?: string }) => entry.type === "leader_nudge_tick",
			);
	...
	it("runs leader nudge checks from canonical fallback when coarse team-state is inactive", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-leader-nudge-canonical-inactive-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		try {
	...
	it("ignores invalid non-authoritative session records before watcher path joins", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-invalid-session-id-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		try {
			...
			await writeFile(
				join(wd, ".omx", "state", "sessions", "safe-session", "session.json"),
				JSON.stringify(
					{
						session_id: "../escape",
						cwd: wd,
			...
			assert.match(
				tmuxLog,
				/send-keys -t %42/,
				"invalid non-authoritative session state must not retarget canonical Team handling",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("skips fallback watcher leader nudges when the leader is not stale even if mailbox messages exist", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-leader-nudge-fresh-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		try {
			...
			hardenTestAuthorityTreeSync(wd);
			await mkdir(
				join(wd, ".omx", "state", "team", "dispatch-team", "mailbox"),
				{ recursive: true },
			);
			hardenTestAuthorityTreeSync(wd);
			await mkdir(fakeBinDir, { recursive: true });
	...
	it("does not run stalled-worker leader nudges from the fallback watcher when the leader is not stale", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-worker-stall-nudge-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		try {
			await installWatcherFixtureAuthority(wd, "sess-worker-nudge");

			hardenTestAuthorityTreeSync(wd);
			await writeCanonicalWatcherTeamFixture(wd, {
				teamName: "dispatch-team",
				sessionId: "sess-worker-nudge",
				ownerSessionId: "sess-worker-nudge",
				coarseState: "active",
			});
			await mkdir(join(wd, ".omx", "logs"), { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			...
			hardenTestAuthorityTreeSync(wd);
			await mkdir(join(wd, ".omx", "state", "team", "dispatch-team", "tasks"), {
				recursive: true,
			});
			hardenTestAuthorityTreeSync(wd);
			await mkdir(fakeBinDir, { recursive: true });
...
`;
			...
			await writeFile(
				join(wd, ".omx", "state", "team-leader-nudge.json"),
				JSON.stringify(
					{
						last_nudged_by_team: {
							"dispatch-team": {
	...
	it("auto-nudges stalled session output even when no active mode state exists", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-auto-nudge-stalled-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const codexHome = join(wd, "codex-home");
		try {
			...
			assert.match(
				watcherState.fallback_auto_nudge?.last_nudged_at ?? "",
				/^\d{4}-\d{2}-\d{2}T/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("respects `.omx/tmux-hook.json` enabled:false for fallback auto-nudge", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-auto-nudge-disabled-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const codexHome = join(wd, "codex-home");
	...
	it("suppresses fallback unmanaged-session auto-nudge skip logs while idle", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-auto-nudge-unmanaged-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const codexHome = join(wd, "codex-home");
		try {
			...
			await mkdir(join(wd, ".omx", "logs"), { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			await mkdirRaw(join(wd, ".omx", "state"), { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			await mkdir(fakeBinDir, { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			await mkdir(codexHome, { recursive: true });
			...
			const tmuxHookLogPath = join(
				wd,
				".omx",
				"logs",
				`tmux-hook-${new Date().toISOString().split("T")[0]}.jsonl`,
			);
	...
	it("does not fallback auto-nudge a stalled hud snapshot that notify-hook already nudged", async () => {
		...
		try {
			...
			await writeFile(
				join(wd, ".omx", "state", "auto-nudge-state.json"),
				JSON.stringify(
					{
						nudgeCount: 1,
	...
	it("does not fallback auto-nudge the same stalled hud turn again after TTL expiry", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-auto-nudge-exact-dedup-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const codexHome = join(wd, "codex-home");
	...
	it("skips dispatch drain in worker context (leader-only guard)", async () => {
		...
		try {
			...
			assert.equal(
				watcherState.dispatch_drain?.last_result?.reason,
				"worker_context",
			);
	...
	it("sends bounded periodic Ralph continue steer while Ralph state stays active", async () => {
		...
		try {
			...
			assert.equal(
				sends.length,
				1,
				"cadence should suppress a second Ralph steer inside 60s",
			);
	...
	it("suppresses Ralph continue steer when hud progress is still fresh after cooldown", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-progress-fresh-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			assert.equal(
				sends.length,
				0,
				"fresh progress should suppress continue steer even after cooldown elapses",
			);
	...
	it("still sends Ralph continue steer when hud progress is stale after cooldown", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-progress-stale-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			assert.equal(
				sends.length,
				1,
				"stale progress should still allow continue steer once cooldown elapses",
			);
	...
	it("suppresses Ralph continue steer when session-scoped Ralph is stuck in stale starting phase", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-starting-stale-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			...
			await writeFile(
				join(sessionStateDir, "hud-state.json"),
				JSON.stringify(
					{
			...
			assert.equal(
				sends.length,
				0,
				"stale starting phase should suppress continue steer",
			);
	...
	it("suppresses Ralph continue steer while tracked native subagents are still active", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-subagents-active-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			await installWatcherFixtureAuthority(wd, omxSessionId);

			hardenTestAuthorityTreeSync(wd);
			await mkdir(join(stateDir, "sessions", omxSessionId), {
				recursive: true,
			});
			hardenTestAuthorityTreeSync(wd);
			await mkdir(fakeBinDir, { recursive: true });
			...
			await writeFile(join(fakeBinDir, "tmux"), buildFakeTmux(tmuxLogPath));
			await chmod(join(fakeBinDir, "tmux"), 0o755);
			await writeSessionStartWithAuthority(wd, omxSessionId, {
				nativeSessionId: codexSessionId,
				ownerOmxSessionId: omxSessionId,
			});
			const refreshedAuthority = await resolveStateAuthorityForGuard({
				startup_cwd: wd,
				observed_cwd: wd,
			...
			await mintStateAuthorityTransportCapability(refreshedAuthority);
			const refreshedTransport = buildStateAuthorityTransportEnv(
				refreshedAuthority,
				{ OMX_SESSION_ID: omxSessionId },
			);
			watcherFixtureAuthorityEnvs.set(wd, refreshedTransport);
			Object.assign(process.env, refreshedTransport);
			await writeFile(
				join(stateDir, "sessions", omxSessionId, "ralph-state.json"),
				JSON.stringify(
					{
						active: true,
						current_phase: "executing",
			...
			await writeFile(
				join(stateDir, "sessions", omxSessionId, "hud-state.json"),
				JSON.stringify(
					{
						last_progress_at: new Date(Date.now() - 61_000).toISOString(),
					},
			...
			await writeFile(
				join(stateDir, "sessions", omxSessionId, "subagent-tracking.json"),
				JSON.stringify(
					{
						schemaVersion: 1,
						sessions: {
			...
			const env = buildWatcherFixtureEnv(wd, {
				PATH: `${fakeBinDir}:${process.env.PATH || ""}`,
			});
			...
			assert.equal(
				sends.length,
				0,
				"active native subagents should block fallback continue steer",
			);
			...
			assert.equal(
				watcherState.ralph_continue_steer?.subagent_session_id,
				codexSessionId,
			);
			assert.deepEqual(
				watcherState.ralph_continue_steer?.active_subagent_thread_ids,
				["sub-thread-1"],
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
	...
	it("fails closed when Ralph hud progress is missing or invalid", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-progress-guard-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			assert.equal(
				missingRun.status,
				0,
				missingRun.stderr || missingRun.stdout,
			);
			...
			assert.equal(
				invalidRun.status,
				0,
				invalidRun.stderr || invalidRun.stdout,
			);
			...
			assert.equal(
				sends.length,
				0,
				"missing or invalid progress should fail closed without sending steer",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("fails closed when active Ralph state has no bound tmux pane", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-pane-missing-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			const tmuxLog = await readFile(tmuxLogPath, "utf8").catch(() => "");
			assert.equal(
				/send-keys -t %42 -l Ralph loop active continue \[OMX_TMUX_INJECT\]/.test(
					tmuxLog,
				),
				false,
			);
			assert.equal(
				/display-message -p -t %42 #{pane_id}/.test(tmuxLog),
				false,
				"watcher should not guess a pane when tmux_pane_id is missing",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("rebinds a stale-but-present session-scoped Ralph shell pane to the live pane before continue steer", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-rebind-stale-anchor-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			await writeFile(
				...
				buildManagedRalphTmux(tmuxLogPath, {
					cwd: wd,
					managedSessionName,
					anchorPane,
					livePane,
			...
			const persistedRalph = JSON.parse(
				await readFile(ralphStatePath, "utf-8"),
			);
			assert.equal(persistedRalph.tmux_pane_id, livePane);
			assert.match(
				persistedRalph.tmux_pane_set_at ?? "",
				/^\d{4}-\d{2}-\d{2}T/,
			);
			...
			assert.match(
				tmuxLog,
				/send-keys -t %42 -l Ralph loop active continue \[OMX_TMUX_INJECT\]/,
			);
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %99 -l Ralph loop active continue \[OMX_TMUX_INJECT\]/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("preserves newer Ralph state fields when a pane rebound happens after the state file advances", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-rebind-state-merge-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
...
`;
			...
			assert.equal(
				persistedRalph.owner_codex_session_id,
				"codex-updated-owner",
			);
			assert.match(
				persistedRalph.tmux_pane_set_at ?? "",
	...
	it("keeps the verified Ralph anchor pane when another codex pane is focused in the same managed session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-keep-anchor-pane-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
	...
	it("rebinds a shell-degraded codex anchor to the live pane before continue steer", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-rebind-degraded-codex-anchor-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
	...
	it("falls back to the current managed session pane when the stored Ralph pane anchor is dead", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-rebind-dead-anchor-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
	...
	it("sends the first Ralph continue steer immediately when persisted steer state is empty", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-startup-cooldown-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			assert.equal(
				sends.length,
				1,
				"empty startup state should send the first Ralph steer immediately once progress is stale",
			);
	...
	it("falls back to an aged persisted cooldown anchor when last_sent_at is invalid", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-invalid-last-sent-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const stateDir = join(wd, ".omx", "state");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			...
			assert.equal(
				sends.length,
				1,
				"invalid last_sent_at should fall back to the persisted cooldown anchor once 60s have elapsed",
			);
	...
	it("treats blocked_on_user as terminal so Ralph continue steer stays off", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-blocked-on-user-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			...
			assert.equal(
				sends.length,
				0,
				"blocked_on_user should suppress Ralph continue steer",
			);
	...
	it("stops Ralph continue steer immediately once Ralph state is terminal or cleared", async () => {
		...
		try {
			...
			watcherState.ralph_continue_steer.last_sent_at = new Date(
				Date.now() - 61_000,
			).toISOString();
			...
			assert.equal(
				terminalRun.status,
				0,
				terminalRun.stderr || terminalRun.stdout,
			);
			...
			assert.equal(
				clearedRun.status,
				0,
				clearedRun.stderr || clearedRun.stdout,
			);
			...
			assert.equal(
				sends.length,
				1,
				"terminal/cleared Ralph state must stop additional periodic steer sends",
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("treats a long-running starting phase as terminal so Ralph steer stops", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-starting-phase-stale-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			...
			assert.equal(
				sends.length,
				0,
				"stale starting phase should block Ralph continue steer",
			);
	...
	it("globally debounces Ralph continue steer across concurrent watcher instances", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-ralph-global-debounce-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			...
			assert.equal(
				sends.length,
				1,
				"shared timestamp + lock should allow only one concurrent Ralph steer send",
			);

			const sharedTimestamp = (
				await readFile(sharedTimestampPath, "utf-8")
			).trim();
			assert.match(
				sharedTimestamp,
				/^\d{4}-\d{2}-\d{2}T/,
				"concurrent send winner should persist a shared cooldown timestamp",
			);

			const watcherState = JSON.parse(
				await readFile(join(stateDir, "notify-fallback-state.json"), "utf-8"),
			);
			assert.equal(
				watcherState.ralph_continue_steer?.shared_timestamp_path,
				sharedTimestampPath,
			);
			assert.equal(
				watcherState.ralph_continue_steer?.shared_last_sent_at,
				sharedTimestamp,
			);
			assert.match(
				watcherState.ralph_continue_steer?.last_reason ?? "",
	...
	it("keeps team control-plane pumping when Ralph continue steer fails", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-control-plane-split-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const previousRuntimeBridge = process.env.OMX_RUNTIME_BRIDGE;
		try {
			...
			assert.match(
				watcherState.ralph_continue_steer?.last_error ?? "",
				/send failed/i,
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf8");
			...
			const ralphFailureEvent = logEntries.find(
				(entry: { type?: string; reason?: string }) =>
					entry.type === "ralph_continue_steer" &&
					entry.reason === "send_failed",
			);
			assert.ok(
				ralphFailureEvent,
				"expected Ralph failure to be logged without aborting team control-plane pumping",
			);
		} finally {
			if (typeof previousRuntimeBridge === "string")
				process.env.OMX_RUNTIME_BRIDGE = previousRuntimeBridge;
	...
	it("prints notify script missing errors to stderr for authority-only ticks", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-authority-missing-script-"),
		);
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-authority-missing-home-"),
		);
		const watcherScript = new URL(
			"../../../dist/scripts/notify-fallback-watcher.js",
			import.meta.url,
		).pathname;
		const missingNotifyHook = join(
			wd,
			"dist",
			"scripts",
			"missing-notify-hook.js",
		);
		try {
			hardenTestAuthorityTreeSync(wd);
			const run = spawnSync(
			...
			assert.match(
				run.stderr,
				/notify-fallback-watcher: notify script missing:/,
			);
	...
	it("prints fatal watcher errors to stderr for authority-only ticks", async () => {
		...
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-authority-fatal-home-"),
		);
		const watcherScript = new URL(
			"../../../dist/scripts/notify-fallback-watcher.js",
			import.meta.url,
		...
		try {
			...
			assert.match(
				run.stderr,
				/notify-fallback-watcher: fatal: test fatal notify fallback failure/,
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
			await rm(tempHome, { recursive: true, force: true });
	...
	it("ignores stale session-scoped Ralph state when the current session identity is stale", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-stale-session-ralph-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			...
			assert.equal(
				sends.length,
				0,
				"stale current-session identity must block Ralph continue injection",
			);
	...
	it("ignores stale root Ralph state when the current session has not started Ralph", async () => {
		...
		try {
			...
			assert.equal(
				sends.length,
				0,
				"fresh sessions must ignore stale root Ralph state",
			);
	...
	it("keeps ticking for active session-scoped Ralph after parent loss, then stops once Ralph is terminal", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-ralph-active-"),
		);
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-ralph-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			...
			await waitFor(
				async () => {
					...
					return /send-keys -t %42 -l Ralph loop active continue \[OMX_TMUX_INJECT\]/.test(
						tmuxLog,
					);
				},
				4000,
			...
			assert.ok(
				...
				"expected watcher to stay alive while Ralph remains active",
			);
			...
			assert.ok(
				logEntries.some(
					(entry: { type?: string; reason?: string }) =>
						entry.type === "watcher_parent_guard" &&
						entry.reason === "parent_gone_deferred_for_active_ralph",
				),
			);
			assert.ok(
				logEntries.some(
	...
	it("stays alive after parent exit while an active team still has live worker panes", async () => {
		...
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-team-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		...
		try {
			await installWatcherFixtureAuthority(wd, "sess-parent-team");

			hardenTestAuthorityTreeSync(wd);
			await writeCanonicalWatcherTeamFixture(wd, {
				teamName: "dispatch-team",
				sessionId: "sess-parent-team",
				ownerSessionId: "sess-parent-team",
				coarseState: "active",
			});
			await mkdir(join(wd, ".omx", "logs"), { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			...
			await waitFor(
				async () => {
					...
					return logEntries.some(
						(entry: { type?: string; reason?: string }) =>
							entry.type === "watcher_parent_guard" &&
							entry.reason === "parent_gone_deferred_for_active_team",
					);
				},
				4000,
				50,
			...
			assert.ok(
				isPidAlive(child.pid),
				"expected watcher to stay alive while team worker panes remain active",
			);
			...
			await writeFile(
				join(wd, ".omx", "state", "team", "dispatch-team", "phase.json"),
				JSON.stringify(
					{
						current_phase: "complete",
						completed_at: new Date().toISOString(),
	...
	it("stays alive after parent exit when coarse team-state is missing but canonical team is active for the current session", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-canonical-team-"),
		);
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-canonical-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		let child: ReturnType<typeof spawn> | undefined;
		...
		try {
			await installWatcherFixtureAuthority(wd, "sess-parent-canonical");
			...
			assert.ok(
				isPidAlive(child.pid),
				"expected watcher to stay alive while canonical team panes remain active",
			);
		} finally {
			if (child && isPidAlive(child.pid)) {
				child.kill("SIGTERM");
	...
	it("does not defer parent-loss shutdown when canonical owner session is blank and coarse team-state is missing", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-ownerless-team-"),
		);
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-ownerless-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		let child: ReturnType<typeof spawn> | undefined;
	...
	it("rejects invalid session_id before resolving session-scoped team paths", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-invalid-session-team-path-"),
		);
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-invalid-session-team-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
		...
		try {
			await installWatcherFixtureAuthority(wd, "sess-valid");
			hardenTestAuthorityTreeSync(wd);
			await mkdir(join(wd, ".omx", "logs"), { recursive: true });
			hardenTestAuthorityTreeSync(wd);
			...
			await writeFile(
				join(maliciousTeamDir, "team-state.json"),
				JSON.stringify(
					{
						active: true,
						team_name: "dispatch-team",
			...
			await writeFile(
				join(maliciousTeamDir, "config.json"),
				JSON.stringify(
					{
						name: "dispatch-team",
						tmux_session: "dispatch-team:0",
			...
			assert.equal(
				child.exitCode,
				1,
				"tampering with the authenticated session anchor must fail closed",
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf-8").catch(() => "");
			assert.equal(
				tmuxLog.trim(),
				"",
				"invalid authenticated session state must be rejected before Team path resolution",
			);
		} finally {
			if (child && isPidAlive(child.pid)) {
				child.kill("SIGTERM");
	...
	it("rejects invalid team_name before resolving watcher team paths", async () => {
		...
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-invalid-team-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		const stateDir = join(wd, ".omx", "state");
	...
	it("does not defer parent-loss shutdown for a team that is already terminal in phase.json", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-terminal-team-"),
		);
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-parent-gone-terminal-team-home-"),
		);
		const fakeBinDir = join(wd, "fake-bin");
		const tmuxLogPath = join(wd, "tmux.log");
		let child: ReturnType<typeof spawn> | undefined;
	...
	it("backs off idle polling and resets to the base cadence after fresh rollout activity", async () => {
		...
		const tempHome = await mkdtemp(
			join(tmpdir(), "omx-fallback-idle-backoff-home-"),
		);
		const sid = randomUUID();
		const sessionDir = todaySessionDir(tempHome);
		const rolloutPath = join(
			sessionDir,
			`rollout-test-fallback-idle-backoff-${sid}.jsonl`,
		);
		const watcherScript = new URL(
			"../../../dist/scripts/notify-fallback-watcher.js",
			import.meta.url,
...
`,
			...
			child = spawn(
				...
				],
				{
					...
					env: buildWatcherFixtureEnv(wd, {
						HOME: tempHome,
						OMX_NOTIFY_FALLBACK_IDLE_MAX_POLL_MS: "200",
					}),
				},
			);
			...
			await waitFor(
				async () => {
					try {
						...
						return (
							watcherState.adaptive_poll?.current_ms === 200 &&
							watcherState.adaptive_poll?.idle_streak >= 2
						);
					} catch {
						return false;
					}
			...
			await waitFor(
				async () => {
					...
					return (
						watcherState.adaptive_poll?.current_ms === 50 &&
	...
	it("exits after the configured max lifetime", async () => {
		...
		try {
			...
			assert.ok(
				logEntries.some(
					(entry: { type?: string; reason?: string }) =>
						entry.type === "watcher_stop" &&
						entry.reason === "max_lifetime_exceeded",
				),
			);
		} finally {
			if (child && isPidAlive(child.pid)) {
	...
	it("keeps the detached helper alive after the hidden bootstrap exits", async () => {
		const wd = await mkdtemp(
			join(tmpdir(), "omx-fallback-bootstrap-survival-"),
		);
...
`,
			...
			assert.ok(
				Number.isFinite(helperPid) && helperPid > 0,
				"expected detached helper pid from bootstrap",
			);

			await waitFor(
				async () => {
					try {
						const ready = JSON.parse(await readFile(readyPath, "utf-8")) as {
							pid?: number;
						};
			...
			assert.ok(
				...
				"expected detached helper to survive after bootstrap exit",
			);
