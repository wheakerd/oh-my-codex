...
describe('notify-hook native dispatch contract', () => {
  it('force-enables hook dispatch for notify-hook native and derived events', async () => {
    ...
    assert.match(source, /dispatchHookEvent\(event, \{ cwd, stateRoot: hookStateRoot \}\);/);
    assert.match(source, /dispatchHookEvent\(derivedEvent, \{ cwd, stateRoot: hookStateRoot \}\);/);
    const matches = source.match(/dispatchHookEvent\(event, \{ cwd, stateRoot: hookStateRoot \}\);/g) ?? [];
