...
} from "../../scripts/notify-hook/team-leader-nudge.js";
import {
...
async function initializeNotifyFixtureAuthority(
	cwd: string,
	sessionId = "notify-team-leader-nudge",
): Promise<void> {
	...
	const authority = await initializeStateAuthority({
		startup_cwd: cwd,
		observed_cwd: cwd,
		launch_id: `notify-team-leader-nudge-${sessionId}-${Date.now()}`,
...
function currentLinuxIdentity(): { startTicks?: number; cmdline?: string } {
	...
	try {
		...
		const cmdline = readFileSync(`/proc/${process.pid}/cmdline`)
			.toString("utf8")
			.replace(/\0+/g, " ")
			.trim();
		return {
...
async function withFixtureAuthorityEnv(
	cwd: string,
	run: () => Promise<void>,
): Promise<void> {
	const authorityEnv = Object.fromEntries(
		Object.entries(fixtureAuthorityEnv.get(cwd) ?? {}).filter(
			(entry): entry is [string, string] => typeof entry[1] === "string",
		),
	);
	await withProcessEnv(authorityEnv, run);
}
...
async function withTempWorkingDir(
	...
	try {
		...
		await withFixtureAuthorityEnv(cwd, () => run(cwd));
	} finally {
...
async function writeJson(path: string, value: unknown): Promise<void> {
	const persisted =
		/(?:-state|config|manifest\.v2)\.json$/.test(path) &&
		value &&
		typeof value === "object"
			? {
					session_id: "notify-team-leader-nudge",
					owner_session_id: "notify-team-leader-nudge",
					owner_codex_session_id: "notify-team-leader-nudge",
...
async function writeCanonicalTeamFixture(
	...
	await writeJson(join(teamDir, "manifest.v2.json"), {
		...
		leader: {
			session_id: ownerSessionId ? "notify-team-leader-nudge" : "",
			worker_id: "leader-fixed",
			role: "coordinator",
		},
		policy: {
...
async function writeLeaderNudgeRaceFixture(
	cwd: string,
	teamName: string,
): Promise<void> {
	const stateDir = join(
	...
	await writeJson(join(teamDir, "tasks", "task-1.json"), {
		id: "1",
		subject: "Pending race work",
		description: "keep the Team active until the injected shutdown race",
		status: "pending",
	...
	it("does not inject leader nudge from notify-hook when team is active and stale", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/send-keys/,
				"current implementation nudges the leader directly in this stale-leader path",
			);
		});
	});
...
describe("notify-hook team leader nudge", { concurrency: false }, () => {
	it("disables leader nudges when deep-interview state is active", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			const fakeTmuxPath = join(fakeBinDir, "tmux");
			const tmuxLogPath = join(cwd, "tmux.log");
			const sessionId = "notify-team-leader-nudge";
			...
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %97 -l Team deep-interview-suppressed:/,
			);
		});
	});
	...
	it("sends immediate all-workers-idle nudge for active team (leader context)", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/\[OMX\] All 2 workers idle/,
				"should emit all-workers-idle nudge",
			);
			assert.doesNotMatch(
				tmuxLog,
				/\[OMX_INTENT:/,
				"should keep orchestration intent out of injected display text",
			);
			assert.match(
				tmuxLog,
				/\[OMX_TMUX_INJECT\]/,
			...
			assert.equal(
				...
				2,
				"leader nudge should submit with isolated double C-m",
			);
			assert.ok(
				!/send-keys[^\n]*-l[^\n]*C-m/.test(tmuxLog),
				"must not mix literal payload with submit keypresses",
			);
	...
	it("suggests shutdown when all workers are idle and the current task set is complete", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/Next: decide whether to reconcile\/merge results or gracefully shut down: omx team shutdown idle-shutdown\./,
			);
	...
	it("suggests reusing the team when follow-up tasks are pending and worker panes are still reusable", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/Team idle-followup-reuse has idle workers ready\./,
			);
			assert.match(
				tmuxLog,
	...
	it("suggests launching a new team when follow-up tasks are pending but worker panes are no longer reusable", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/Team idle-followup-relaunch has follow-up work ready\./,
			);
	...
	it("falls back to global team-state when session-scoped state is active but team-state.json remains global", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/\[OMX\] All 2 workers idle/,
				"global team-state fallback should still fire idle nudge",
			);
		});
	});
	...
	it("falls back to canonical team state when coarse team-state is missing", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeCanonicalTeamFixture(cwd, {
				...
				sessionId: "notify-team-leader-nudge",
				ownerSessionId: "notify-team-leader-nudge",
			});
			await writeFile(fakeTmuxPath, buildFakeTmux(tmuxLogPath));
			await chmod(fakeTmuxPath, 0o755);
			...
			assert.match(
				tmuxLog,
				/\[OMX\] All 2 workers idle/,
				"canonical fallback should still fire idle nudge",
			);
		});
	});
	...
	it("does not let stale root deep-interview state suppress session-scoped leader nudges", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeJson(join(cwd, ".omx", "state", "deep-interview-state.json"), {
				active: true,
			...
			assert.match(
				tmuxLog,
				/\[OMX\] All 2 workers idle/,
				"session-scoped nudge should ignore stale root deep-interview state",
			);
		});
	});
	...
	it("falls back to canonical team state when coarse team-state is inactive", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/-t %97/,
				"should still target canonical leader pane",
			);
			assert.match(
				tmuxLog,
				/\[OMX\] All 2 workers idle/,
				"inactive coarse state should still fall back canonically",
			);
		});
	});
	...
	it("ignores invalid team_name before canonical leader follow-up team path joins", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeJson(join(teamRoot, "..-bad-team", "manifest.v2.json"), {
				...
				next_task_id: 1,
				workers: [
					{ name: "worker-1", index: 1, pane_id: "%666", role: "executor" },
				],
				created_at: new Date().toISOString(),
				policy: {
					worker_launch_mode: "interactive",
			...
			assert.match(
				tmuxLog,
				/-t %97/,
				"should still target the valid canonical leader pane",
			);
			assert.doesNotMatch(
				tmuxLog,
				/%666/,
				"invalid canonical team names must be ignored before joins",
			);
			assert.match(
				tmuxLog,
				/\[OMX\] All 2 workers idle/,
				"valid canonical fallback should still fire idle nudge",
			);
		});
	});
...
`,
			...
			assert.equal(
				existsSync(teamDir),
				false,
				"teardown race should remove the canonical team state",
			);
			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %91 -l Team leader-nudge-teardown-race:/,
			);
	...
	it("does not persist bookkeeping when team is removed after the final liveness check", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeFile(
				fakeTmuxPath,
				buildFakeTmuxWithListPanes(tmuxLogPath, ["%11 12345"]),
			);
			await chmod(fakeTmuxPath, 0o755);
			...
			assert.equal(
				existsSync(teamDir),
				false,
				"test seam should remove canonical team state during persistence",
			);
			assert.equal(
				existsSync(join(teamDir, "leader-attention.json")),
				false,
	...
	it("rolls back team nudge bookkeeping when shutdown wins immediately before leader-attention write", {
		concurrency: false,
	}, async () => {
		await withTempWorkingDir(async (cwd) => {
			const stateDir = join(
			...
			await writeFile(
				fakeTmuxPath,
				buildFakeTmuxWithListPanes(join(cwd, "tmux.log"), [
					"%99 12345",
					"%11 12346",
				]),
			);
			...
			await writeJson(join(stateDir, "team-leader-nudge.json"), {
				...
				last_nudged_by_team: {
					[preservedTeam]: { at: "2026-02-14T00:00:00.000Z" },
				},
			...
			try {
				setLeaderNudgeTestHooksForTests({
					beforeLeaderAttentionRename: async ({
						...
						await rm(join(selectedStateDir, "team", teamName), {
							recursive: true,
							force: true,
						});
					},
			...
			assert.equal(
				existsSync(join(stateDir, "team", teamName, "leader-attention.json")),
				false,
			);
			...
			assert.deepEqual(nudgeState.progress_by_team?.[preservedTeam], {
				signature: "keep",
			});
			assert.equal(
				nudgeState.last_nudged_by_team?.[preservedTeam]?.at,
				"2026-02-14T00:00:00.000Z",
			);
			assert.equal(
				nudgeState.last_idle_nudged_by_team?.[preservedTeam]?.at,
				"2026-02-14T00:00:00.000Z",
			);
		});
	});

	it("rolls back team nudge bookkeeping when shutdown wins during leader-attention write", {
		concurrency: false,
	}, async () => {
		await withTempWorkingDir(async (cwd) => {
			const stateDir = join(
			...
			try {
				setLeaderNudgeTestHooksForTests({
					afterLeaderAttentionRename: async ({
						stateDir: selectedStateDir,
					}: {
						stateDir: string;
					}) => {
...
`;
			...
			assert.ok(
				tmuxLog.indexOf("send-keys -t %93 Tab") <
					tmuxLog.indexOf("send-keys -t %93 C-m"),
	...
	it("surfaces ack-like mailbox replies without work-start evidence as missing-start nudges", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/Next: check worker-1 msg\/output, confirm task in omx team status ack-missing-start/,
			);
			...
			const nudgeEvent = events.find(
				(e: {
					...
					e.type === "team_leader_nudge" &&
					e.reason === "ack_without_start_evidence",
			);
			assert.ok(
				nudgeEvent,
				"should emit an ack_without_start_evidence leader nudge",
			);
	...
	it("does not classify ack-like replies as missing-start after a worker has claimed work", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/Next: read messages; keep orchestrating; if done, gracefully shut down: omx team shutdown ack-with-start\./,
			);
	...
	it("does not re-nudge for the same fresh mailbox message on repeated notify-hook runs", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			const first = runNotifyHook(cwd, fakeBinDir, {
				OMX_TEAM_LEADER_NUDGE_MS: "600000",
			});
			assert.equal(
				first.status,
				0,
				`notify-hook failed: ${first.stderr || first.stdout}`,
			);
			...
			assert.equal(
				second.status,
				0,
				`notify-hook failed: ${second.stderr || second.stdout}`,
			);

			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			const sends =
				tmuxLog.match(
					/send-keys -t %97 -l Team fresh-mailbox-bounded: 1 msg\(s\) for leader\./g,
				) || [];
			assert.equal(
				sends.length,
				1,
				"same mailbox message should not trigger repeated non-stale nudges",
			);
		});
	});
...
`;
			...
			assert.match(
				tmuxLog,
				/display-message -p -t %71 #\{pane_current_command\}/,
			);
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %71/,
				"should not inject into a shell pane",
			);
...
`;
			...
			assert.match(
				tmuxLog,
				/send-keys -t %73/,
				"should inject into a busy leader pane so Codex can queue the message",
			);
			assert.match(tmuxLog, /send-keys -t %73 Tab/);
			assert.match(tmuxLog, /send-keys -t %73 C-m/);
			assert.ok(
				tmuxLog.indexOf("send-keys -t %73 Tab") <
					tmuxLog.indexOf("send-keys -t %73 C-m"),
				"busy leader queue path should press Tab before C-m",
			);
			...
			if (existsSync(eventsPath)) {
				...
				assert.equal(
					deferred,
					undefined,
					"busy leader pane should no longer defer notifications",
				);
			}
		});
	});
...
`;
			...
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %75 -l Team same-classified-state: 1 msg\(s\) for leader\./,
				"same visible classified state should not be reinjected",
			);
			...
			assert.ok(
				nudgeEvent,
				"suppressed visible sends should still emit leader nudge events",
			);
				...
				deliveryLog.some(
					(entry) =>
						...
						entry.suppression_reason ===
							"pane_already_shows_same_classified_state",
				),
			);
		});
	});
...
`;
			...
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %72/,
				"should not inject into a scrolling leader pane",
			);
			...
			const deferred = events.find(
				(entry: { type?: string; reason?: string }) =>
					entry.type === "leader_notification_deferred" &&
					entry.reason === "scroll_active",
			);
			assert.ok(
				deferred,
				"should emit deferred event for scrolling leader pane",
			);
		});
	});

	it("does not nudge a terminal team based on its canonical session phase", async () => {
		await withTempWorkingDir(async (cwd) => {
			const omxDir = join(cwd, ".omx");
			const stateDir = join(
				omxDir,
			...
			assert.equal(
				...
				false,
				"terminal detection must reconcile canonical session state",
			);
			assert.equal(canonicalState.current_phase, "complete");
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys/,
					"must not nudge a terminal team",
				);
			}
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys/,
					"completed teams must not re-nudge on reopen",
				);
			}
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys/,
					"must not nudge teams owned by another session",
				);
			}
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys/,
					"ownerless canonical teams must not nudge",
				);
			}
		});
	});
	...
	it("nudges when worker panes are alive and leader is stale (no recent HUD turn)", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/leader stale, \d+ worker pane\(s\) still active\./,
			);
			assert.match(
				tmuxLog,
				/Next: check messages; keep orchestrating; if done, gracefully shut down: omx team shutdown beta\./,
			);
			assert.doesNotMatch(tmuxLog, /keep polling/);
			assert.match(
				tmuxLog,
	...
	it("does not nudge when only team progress-stall heuristics fire (former fallback threshold)", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			const events = eventsRaw.trim()
				...
				: [];
			const nudgeEvent = events.find(
				(e: { type?: string }) => e.type === "team_leader_nudge",
			);
	...
	it("does not nudge stalled team when an in-progress worker is still advancing heartbeat turns", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.doesNotMatch(
				tmuxLog,
				/active-turns-no-stall: worker panes stalled, no progress/,
			);
			assert.doesNotMatch(
				tmuxLog,
				/active-turns-no-stall: leader stale, no progress/,
			);
		});
	});
	...
	it("bounds repeated stalled-team nudges before leader stale by cooldown", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			const sends =
				tmuxLog.match(
					/send-keys -t %88 -l Team stalled-before-stale-bounded: worker panes stalled, no progress/g,
				) || [];
			assert.equal(
				sends.length,
				0,
				"stall/progress heuristics should not produce leader nudges",
			);
			assert.doesNotMatch(tmuxLog, /worker panes stalled/);
		});
	});
	...
	it("does not treat leader and HUD panes as active worker panes when worker pane ids are known", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.doesNotMatch(
				tmuxLog,
				/send-keys -t %92 -l Team stale-no-workers: leader stale,/,
			);
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/Team fresh-leader/,
					"non-stale leader should not receive generic periodic follow-up",
				);
			}
		});
	});
	...
	it("uses a 30s cadence for stale leader follow-up nudges", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.equal(
				blocked.status,
				0,
				`notify-hook failed: ${blocked.stderr || blocked.stdout}`,
			);
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					firstLog,
					/Team stale-cadence:/,
					"stale follow-up should be blocked inside the 30s window",
				);
			}
			...
			const allowed = runNotifyHook(cwd, fakeBinDir, {
				OMX_TEAM_LEADER_NUDGE_MS: "30000",
			});
			assert.equal(
				allowed.status,
				0,
				`notify-hook failed: ${allowed.stderr || allowed.stdout}`,
			);
			...
			assert.match(
				finalLog,
				/Team stale-cadence: leader stale, \d+ worker pane\(s\) still active\./,
			);
		});
	});
	...
	it("suppresses stale leader follow-up when detached worktree progress is still fresh", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			await writeJson(
				join(workerWorktree, ".omx", "state", "current-task-baseline.json"),
				{
					version: 1,
					tasks: [],
				},
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/Team fresh-detached-progress: leader stale,/,
				);
			}
		});
	});
	...
	it("emits team_leader_nudge event to events.ndjson when nudge fires", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.ok(
				...
				"events.ndjson should exist after nudge",
			);
			const eventsContent = await readFile(eventsPath, "utf-8");
			const events = eventsContent
				.trim()
			...
			assert.ok(
				nudgeEvent.orchestration_intent,
				"event should record an orchestration intent",
			);
		});
	});
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys -t .*devsess/,
					"must not fall back to session target",
				);
			}
			...
			const deferred = events.find(
				(e: { type?: string; reason?: string }) =>
					e.type === "leader_notification_deferred" &&
					e.reason === "leader_pane_missing_no_injection",
			);
			assert.ok(deferred);
			...
			const nudgeStatePath = join(stateDir, "team-leader-nudge.json");
			assert.ok(
				existsSync(nudgeStatePath),
				"nudge state should still advance on deferred leader visibility",
			);
			const nudgeState = JSON.parse(await readFile(nudgeStatePath, "utf-8"));
			assert.ok(nudgeState.last_nudged_by_team?.[teamName]?.at);
			assert.equal(
				nudgeState.last_nudged_by_team?.[teamName]?.orchestration_intent,
				"pending-mailbox-review",
			);

			const leaderAttentionPath = join(
				stateDir,
				"team",
				teamName,
			...
			assert.ok(
				existsSync(leaderAttentionPath),
				"leader attention state should be written from notify-hook",
			);
			const leaderAttention = JSON.parse(
				await readFile(leaderAttentionPath, "utf-8"),
			);
			...
			assert.equal(
				leaderAttention.leader_attention_reason,
				"new_mailbox_message",
			);
			assert.deepEqual(leaderAttention.attention_reasons, [
				"new_mailbox_message",
			]);
	...
	it("bounds repeated all-workers-idle nudges by cooldown", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			const first = runNotifyHook(cwd, fakeBinDir, {
				OMX_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS: "600000",
			});
			assert.equal(
				first.status,
				0,
			...
			const tmuxLog = await readFile(tmuxLogPath, "utf-8");
			const sends =
				tmuxLog.match(/send-keys -t %98 -l \[OMX\] All 2 workers idle/g) || [];
			assert.equal(
				sends.length,
				1,
				"cooldown should keep repeated all-workers-idle leader nudges bounded",
			);
			assert.doesNotMatch(tmuxLog, /\[OMX_INTENT:/);
		});
	});
	...
	it("includes stale_leader_with_messages reason when both conditions met", async () => {
		await withTempWorkingDir(async (cwd) => {
			...
			assert.match(
				tmuxLog,
				/Team delta: leader stale, \d+ pane\(s\) active, 1 msg\(s\) pending\./,
			);
			assert.match(
				tmuxLog,
				/Next: read messages; keep orchestrating; if done, gracefully shut down: omx team shutdown delta\./,
			);
			assert.doesNotMatch(tmuxLog, /keep polling/);
			assert.match(
				tmuxLog,
		...
		await withTempWorkingDir(async (cwd) => {
			...
			if (existsSync(tmuxLogPath)) {
				...
				assert.doesNotMatch(
					tmuxLog,
					/send-keys/,
					"invalid team_name must not be used for leader follow-up path resolution",
				);
			}
		});
	});
