...
async function assertKeywordMutationAuthority(
  ...
  if (!expectedRootIdentity) {
    keywordStateError(AUTHORITY_DIAGNOSTIC_CODES.authorityMissing, 'keyword state mutation requires the persisted state-root identity');
  }
  const root = resolve(stateDir);
  if (root !== resolve(expectedRootIdentity.canonical_path)) {
    keywordStateError(AUTHORITY_DIAGNOSTIC_CODES.workspaceMismatch, 'keyword state directory does not match the persisted state-authority root');
  }
  ...
  if (!sameRootFilesystemIdentity(expectedRootIdentity, actualRootIdentity)) {
    keywordStateError(AUTHORITY_DIAGNOSTIC_CODES.rootFingerprintMismatch, 'keyword state directory does not match the persisted state-root identity');
  }
  return root;
}

async function ensureKeywordStateDirectory(root: string, directory: string, expectedRootIdentity: RootFilesystemIdentity): Promise<void> {
  ...
  for (const component of relativePath ? relativePath.split(/[\\/]+/) : []) {
    ...
    try {
      ...
      if (details.isSymbolicLink() || !details.isDirectory()) {
        keywordStateError(AUTHORITY_DIAGNOSTIC_CODES.authorityPathEscapesRoot, `unsafe keyword state directory: ${current}`);
      }
    } catch (error) {
