...
function resolveSessionPaneTarget(
	sessionName: string,
): HookPluginSendKeysResult {
	const paneList = runTmux([
		...
		sessionName,
		"-F",
		"#{pane_id}\t#{pane_active}\t#{pane_start_command}",
	]);
	if (!paneList.ok) {
	...
	const rows = paneList.stdout
		...
		.filter(Boolean)
		.map((line) => {
			const [paneId = "", activeRaw = "0", startCommand = ""] =
				line.split("\t");
			return {
				paneId,
				active: activeRaw === "1",
	...
	const canonicalRows = nonHudRows.filter((row) =>
		/\bcodex\b/i.test(row.startCommand),
	);
	const resolved =
		canonicalRows.find((row) => row.active) ||
		canonicalRows[0] ||
...
function resolveTmuxTarget(
	...
	const paneId =
		typeof options.paneId === "string" ? options.paneId.trim() : "";
	if (paneId) {
		...
		return {
			...
			error: pane.ok ? "pane_not_found" : pane.stderr,
		};
	}
	...
	if (envPane)
		return { ok: true, reason: "ok", target: envPane, paneId: envPane };
...
async function sendTmuxKeys(
	...
	const tmuxStatePath = hookPluginTmuxStatePath(
		context.cwd,
		context.pluginName,
		context.stateRoot,
	);
	...
	const cooldownMs =
		typeof options.cooldownMs === "number"
			...
			: asPositiveNumber(
					process.env.OMX_HOOK_PLUGIN_COOLDOWN_MS,
					DEFAULT_COOLDOWN_MS,
				);
	const dedupeWindowMs = asPositiveNumber(
		process.env.OMX_HOOK_PLUGIN_DEDUPE_MS,
		DEFAULT_DEDUPE_WINDOW_MS,
	);
	const minTs = now - dedupeWindowMs;

	tmuxState.recent_keys = Object.fromEntries(
		Object.entries(tmuxState.recent_keys || {}).filter(
			([, ts]) => Number.isFinite(ts) && ts >= minTs,
		),
	);
