...
const KNOWN_TYPED_AGENT_ROLES = new Set(
	Object.keys(AGENT_DEFINITIONS).map((role) => role.toLowerCase()),
);

export function subagentTrackingPath(cwd: string, stateDir?: string): string {
	return join(stateDir ?? getBaseStateDir(cwd), "subagent-tracking.json");
}

export function resolveInstalledRoleName(
	role: string,
	...
	for (const agentsDir of [
		codexAgentsDir(codexHomeOverride),
		projectCodexAgentsDir(),
	]) {
		try {
			for (const entry of readdirSync(agentsDir, { withFileTypes: true })) {
				if (!entry.isFile() || !entry.name.endsWith(".toml")) continue;
				const installedRole = entry.name
					.slice(0, -".toml".length)
					.trim()
					.toLowerCase();
...
function normalizeSubagentStatus(
	value: unknown,
): SubagentAvailabilityStatus | undefined {
	const normalized =
...
function compareResumeEntries(
	left: SubagentLedgerEntry,
	right: SubagentLedgerEntry,
): number {
	...
	const lastSeenComparison = compareOptionalTimestampDesc(
		left.lastSeenAt,
		right.lastSeenAt,
	);
	if (lastSeenComparison !== 0) return lastSeenComparison;

	const leftCompletedComparison = compareOptionalTimestampDesc(
		left.completedAt,
		right.completedAt,
	);
...
function normalizeLedgerEntry(
	...
	return {
		...
		...(thread.last_handoff_summary
			? { lastHandoffSummary: thread.last_handoff_summary }
			: {}),
		...(thread.resume_requested_at
			? { resumeRequestedAt: thread.resume_requested_at }
			: {}),
		...(thread.resume_completed_at
			? { resumeCompletedAt: thread.resume_completed_at }
			: {}),
		...(thread.resume_failed_at
			? { resumeFailedAt: thread.resume_failed_at }
			: {}),
		...(thread.resume_failure_reason
			? { resumeFailureReason: thread.resume_failure_reason }
			: {}),
	};
}
...
function normalizePendingRoleIntent(value: unknown): PendingRoleIntent | null {
	...
	if (
		...
		!sessionId ||
		!parentThreadId ||
		!Object.hasOwn(candidate, "correlation_token") ||
		!createdAt ||
		!expiresAt
	)
		return null;
	...
	const bindingState =
		candidate.binding_state === "bound" ? "bound" : undefined;
	const boundAt = readOptionalTrimmedString(candidate.bound_at);
	const hasValidBoundAt = Boolean(
		boundAt && Number.isFinite(Date.parse(boundAt)),
	);
...
export function normalizeSubagentTrackingState(
	input: unknown,
	...
	for (const [sessionId, rawSession] of Object.entries(parsed.sessions ?? {})) {
		...
		for (const [threadId, rawThread] of Object.entries(
			(rawSession as TrackedSubagentSession).threads ?? {},
		)) {
			...
			const normalizedThreadId =
				...
				candidate.thread_id.trim().length > 0
			...
			const firstSeenAt =
				...
				candidate.first_seen_at.trim().length > 0
					? candidate.first_seen_at
					: typeof candidate.last_seen_at === "string" &&
							candidate.last_seen_at.trim().length > 0
			...
			threads[normalizedThreadId] = {
				...
				candidate.last_turn_id.trim().length > 0
				...
				candidate.completed_at.trim().length > 0
					...
					: {}),
				...(typeof candidate.last_completed_turn_id === "string" &&
				candidate.last_completed_turn_id.trim().length > 0
					? { last_completed_turn_id: candidate.last_completed_turn_id }
					: {}),
				turn_count:
					typeof candidate.turn_count === "number" &&
					Number.isFinite(candidate.turn_count) &&
				...
				candidate.role.trim().length > 0
					...
					: {}),
				...(typeof candidate.provenance_kind === "string" &&
				candidate.provenance_kind.trim().length > 0
					? { provenance_kind: candidate.provenance_kind.trim() }
					: {}),
				...(typeof candidate.lane_id === "string" &&
				candidate.lane_id.trim().length > 0
					? { lane_id: candidate.lane_id.trim() }
					: {}),
				...
				candidate.agent_nickname.trim().length > 0
					? { agent_nickname: candidate.agent_nickname.trim() }
					: {}),
				...(typeof candidate.completion_source === "string" &&
				candidate.completion_source.trim().length > 0
					? { completion_source: candidate.completion_source }
					: {}),
				...(normalizeSubagentStatus(candidate.status)
					? { status: normalizeSubagentStatus(candidate.status) }
					: {}),
				...(typeof candidate.last_handoff_summary === "string" &&
				candidate.last_handoff_summary.trim().length > 0
					? { last_handoff_summary: candidate.last_handoff_summary.trim() }
					: {}),
				...(typeof candidate.resume_requested_at === "string" &&
				candidate.resume_requested_at.trim().length > 0
					? { resume_requested_at: candidate.resume_requested_at.trim() }
					: {}),
				...(typeof candidate.resume_completed_at === "string" &&
				candidate.resume_completed_at.trim().length > 0
					? { resume_completed_at: candidate.resume_completed_at.trim() }
					: {}),
				...(typeof candidate.resume_failed_at === "string" &&
				candidate.resume_failed_at.trim().length > 0
					? { resume_failed_at: candidate.resume_failed_at.trim() }
					: {}),
				...(typeof candidate.resume_failure_reason === "string" &&
				candidate.resume_failure_reason.trim().length > 0
	...
	const pendingRoleIntents = Array.isArray(parsed.pending_role_intents)
		? parsed.pending_role_intents
				.map((intent) => normalizePendingRoleIntent(intent))
				.filter((intent): intent is PendingRoleIntent => intent !== null)
		: [];
	...
	try {
		...
		const acquiredAtMs =
			typeof parsed.acquired_at === "string"
				? Date.parse(parsed.acquired_at)
				: Number.NaN;
		const pidStartId =
			parsed.pid_start_id === undefined
				? undefined
...
function isCrossProcessLockOlderThanLease(
	acquiredAtMs: number,
	nowMs: number,
): boolean {
...
function isCrossProcessLockReclaimable(claim: CrossProcessLockClaim): boolean {
	...
	if (claim.pidStartId && currentPidStartId)
		return currentPidStartId !== claim.pidStartId;
	return isCrossProcessLockOlderThanLease(claim.acquiredAtMs, Date.now());
}
...
function tryAcquireCrossProcessFileLock(
	lockPath: string,
	token: string,
): boolean {
	...
	try {
		writeFileSync(
			temporaryPath,
			serializeCrossProcessLockClaim(createCrossProcessLockClaim(token)),
		);
		try {
...
function restoreQuarantinedCrossProcessLock(
	lockPath: string,
...
function recoverCrossProcessFileLock(
	...
	if (
		observed.kind === "claim" &&
		!isCrossProcessLockReclaimable(observed.claim)
	)
		return false;
	...
	const capturedExpectedClaim =
		observed.kind === "claim" &&
		captured.kind === "claim" &&
		captured.claim.token === observed.claim.token;
	const capturedRecoverable =
		captured.kind === "malformed" ||
		(capturedExpectedClaim &&
			captured.kind === "claim" &&
			isCrossProcessLockReclaimable(captured.claim));
...
function assertCrossProcessFileLockOwnership(
	lockPath: string,
	token: string,
): void {
...
function crossProcessLockStagePath(
	resourcePath: string,
	token: string,
): string {
...
function sweepForeignCrossProcessLockStages(
	resourcePath: string,
	token: string,
): void {
	...
	for (const entry of readdirSync(directory)) {
		if (
			...
			entry.slice(stagePrefix.length) === token
		)
			continue;
...
function sweepAbandonedCrossProcessLockArtifacts(resourcePath: string): void {
	...
	const artifactPattern = new RegExp(
		`^${escapedLockName}\\.([^.]+)\\.[^.]+\\.(?:quarantine|release)$`,
	);
	...
	for (const entry of readdirSync(directory)) {
		...
		if (
			!Number.isFinite(displacedAtMs) ||
			nowMs - displacedAtMs <= CROSS_PROCESS_LOCK_LEASE_MS
		)
			continue;
	...
	aged.sort(
		(a, b) =>
			a.displacedAtMs - b.displacedAtMs ||
			(a.entry < b.entry ? -1 : a.entry > b.entry ? 1 : 0),
	);
	for (const { entry } of aged.slice(
		0,
		CROSS_PROCESS_LOCK_ARTIFACT_SWEEP_CAP,
	)) {
		removeCrossProcessLockFile(join(directory, entry));
	}
}
...
let crossProcessQuarantineBarrier:
	| ((lockPath: string, quarantinedPath: string) => void)
	| null = null;

export function __setCrossProcessPublishBarrierForTest(
	barrier: (() => void) | null,
): void {
...
export function withCrossProcessFileLockSync<T>(
	...
	for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
		...
		const recovered = recoverCrossProcessFileLock(
			lockPath,
			readCrossProcessLockState(lockPath),
		);
		...
		if (attempt === maxAttempts - 1) {
			throw new Error(
				`Timed out waiting for cross-process lock at ${lockPath}`,
			);
		}
		sleepForCrossProcessLockSync(
			Math.min(25, retryMs * 2 ** Math.min(attempt, 4)),
		);
	}
	...
	try {
		...
		return operation({
			assertOwnership: () =>
				assertCrossProcessFileLockOwnership(lockPath, token),
...
function readSubagentTrackingStateSync(
	cwd: string,
	stateDir?: string,
): SubagentTrackingState {
	...
	try {
		return normalizeSubagentTrackingState(
...
export async function readSubagentTrackingState(
	cwd: string,
	stateDir?: string,
): Promise<SubagentTrackingState> {
	const path = subagentTrackingPath(cwd, stateDir);
	if (!existsSync(path)) return createSubagentTrackingState();
	try {
		return normalizeSubagentTrackingState(
...
export async function writeSubagentTrackingState(
	cwd: string,
	state: SubagentTrackingState,
): Promise<string> {
	const normalized = normalizeSubagentTrackingState(state);
...
function isExpiredPendingRoleIntent(
...
function pendingRoleIntentPredicates(
	cwd: string,
...
function selectDominantRoleIntent(
	...
	return (
		[...candidates].sort((left, right) => {
			...
			const leftIsExactOrigin =
				left.origin_cwd !== undefined &&
				canonicalizeOriginCwd(left.origin_cwd) === canonicalOrigin;
			const rightIsExactOrigin =
				right.origin_cwd !== undefined &&
				canonicalizeOriginCwd(right.origin_cwd) === canonicalOrigin;
			if (leftIsExactOrigin !== rightIsExactOrigin)
				return leftIsExactOrigin ? -1 : 1;
...
export function recordPendingRoleIntent(
	...
	| { ok: true; intent: PendingRoleIntent }
	| {
			ok: false;
			reason:
				| "unknown_role"
	...
	return withCrossProcessFileLockSync(subagentTrackingPath(cwd), (context) => {
		...
		state.pending_role_intents = [
			...all.filter((candidate) => !shouldPruneExpired(candidate)),
			intent,
		];
...
export function bindPendingRoleIntentUnderLock(
	...
	claimantToken: string | undefined;
				...
				(intent) =>
					!(
						...
						sameLogicalRoleIntent(
							...
							sessionId,
							parentThreadId,
							boundIntent.correlation_token as string,
						)
					),
			);
		context.assertOwnership();
...
export function listBoundAdaptedRoleIntents(
	cwd: string,
	_nowMs?: number,
	ownedDominant = false,
): PendingRoleIntent[] {
	const allBound = readSubagentTrackingStateSync(
		cwd,
	).pending_role_intents.filter((intent) => intent.binding_state === "bound");
	if (!ownedDominant) return allBound;
	const canonicalOrigin = canonicalizeOriginCwd(cwd);
	if (canonicalOrigin === null) return [];
	const { isOwn } = pendingRoleIntentPredicates(
		cwd,
		canonicalOrigin,
		Date.now(),
	);
...
export function recordSubagentTurn(
	...
	const requestedLeaderThread = requestedLeaderThreadId
		...
		: undefined;
	const requestedLeaderWouldReclassifySubagent =
		requestedLeaderThread?.kind === "subagent";
	const requestedSessionLeaderThreadId = requestedLeaderWouldReclassifySubagent
		? undefined
		: requestedLeaderThreadId;
	const preserveExistingSubagent =
		existingKind === "subagent" && requestedKind !== "subagent";
	const preserveKnownLeader =
	...
	const leaderThreadId = preserveKnownLeader
		...
		: existingLeaderThreadId ||
			...
			(requestedKind === "subagent" || preserveExistingSubagent
				? undefined
				: threadId);
		...
		preserveCompletionEvidence && existingThread?.completed_at
			? {
					...
					...(existingThread.last_completed_turn_id
						...
						: {}),
					...(existingThread.completion_source
	...
	const nextThread: TrackedSubagentThread = {
		...
		...(input.mode?.trim()
			...
			: existingThread?.mode
				? { mode: existingThread.mode }
				: {}),
		...(input.role?.trim()
			? { role: input.role.trim() }
			: existingThread?.role
				? { role: existingThread.role }
				: {}),
		...(input.provenanceKind?.trim()
			...
			: existingThread?.provenance_kind
				...
				: {}),
		...(input.laneId?.trim()
			? { lane_id: input.laneId.trim() }
			: existingThread?.lane_id
				? { lane_id: existingThread.lane_id }
				: {}),
		...(input.scope?.trim()
...
export async function recordSubagentTurnForSession(
	cwd: string,
	input: RecordSubagentTurnInput,
	stateDir?: string,
): Promise<SubagentTrackingState> {
...
export function summarizeSubagentSession(
	...
	const activeWindowMs =
		options.activeWindowMs ?? DEFAULT_SUBAGENT_ACTIVE_WINDOW_MS;
	const nowMs =
	...
	const allSubagentThreadIds = allThreadIds.filter((threadId) =>
		isTrustedSubagentThread(session, threadId),
	);
	...
	const savedSubagents = allSubagentThreadIds.map(
		(threadId): SubagentResumeEntry => {
			const thread = session.threads[threadId]!;
			const role = thread.role ?? thread.mode;
			const laneId = thread.lane_id ?? thread.agent_nickname ?? role;
			return {
				...
				status:
					thread.status ??
					(activeSubagentThreadIdSet.has(threadId) ? "available" : "closed"),
			};
		},
	);
...
export function buildSubagentResumeLedger(
	...
	const savedSubagents = summary.savedSubagents.map(
	...
	const unavailableSubagents = savedSubagents.filter(
...
export async function readSubagentSessionLedger(
	...
	return buildSubagentResumeLedger(
		await readSubagentTrackingState(cwd),
		sessionId,
		options,
	);
}
...
export function selectReusableSubagentEntry(
	...
	const normalizedAgentNickname = readOptionalTrimmedString(
		criteria.agentNickname,
	);
	...
	const matchingEntries = entries.filter((entry) => {
		...
		if (
			...
			entry.agentNickname !== normalizedAgentNickname
		)
			return false;
		return true;
	});
	...
	const scoredEntries = matchingEntries
		...
		.sort((left, right) => {
			...
			const leftActivity = compareOptionalTimestampDesc(
				left.entry.lastSeenAt,
				right.entry.lastSeenAt,
			);
...
export async function readSubagentSessionSummary(
	...
	return summarizeSubagentSession(
		await readSubagentTrackingState(cwd, options.stateDir),
		sessionId,
		options,
	);
}
