...
function resolveSynchronousTransportStateRoot(
	...
	if (
		!bindingMatchesAuthoritySession(binding, authoritySessionCandidate(env))
	) {
		failSynchronousAuthorityTransport(
			AUTHORITY_DIAGNOSTIC_CODES.sessionBindingConflict,
...
function assertNoAmbientAuthorityAliasConflict(
	...
	for (const [name, value] of TEAM_AMBIENT_STATE_ROOT_ALIAS_ENV_KEYS.map(
...
function parseStrictWorkerIdentity(
	...
	const workingPath =
		metadataStateRoot(identity.worktree_path) ??
...
function identityMatchesManifest(
	...
	const manifestWorker = workers.find(
		(candidate) =>
			candidate &&
	...
	return (
		...
		manifestWorker.assigned_tasks.every(
			(taskId) => typeof taskId === "string",
		) &&
		JSON.stringify(manifestWorker.assigned_tasks) ===
			JSON.stringify(identity.assignedTasks)
	);
}
...
async function validateWorkerStateRoot(
	...
	if (authority) {
		const canonicalSessionId = authority.session_binding?.canonical_session_id;
		const allowedAuthorityRoots = new Set(
			await Promise.all(
				[
		...
		const normalizedCandidate = await normalizePath(resolvedStateRoot);
		if (!allowedAuthorityRoots.has(normalizedCandidate)) {
			return {
				ok: false,
				stateRoot: null,
				source: null,
	...
	if (authority) {
		...
		if (!allowedAuthorityRoots.has(normalizedIdentityRoot)) {
			return {
				ok: false,
				stateRoot: null,
				source: null,
...
async function explicitRootConflictsWithAuthority(
	...
	const allowedRoots = new Set<string>([
		await normalizePath(authority.canonical_state_root),
		...(canonicalSessionId
			? [
					await normalizePath(
						join(
	...
	for (const candidate of explicitStateRootCandidates(cwd, env)) {
		if (!allowedRoots.has(await normalizePath(candidate))) return true;
	}
	return false;
}
...
async function resolveWorkerTeamStateRootWithOptions(
	...
	if (authority.context) {
		...
		const explicitTeamRoot =
			typeof env.OMX_TEAM_STATE_ROOT === "string"
				? env.OMX_TEAM_STATE_ROOT.trim()
				: "";
		const canonicalSessionId =
			authority.context.session_binding?.canonical_session_id;
		const authorityRoots = [
			...new Set([
				...(explicitTeamRoot ? [resolve(cwd, explicitTeamRoot)] : []),
				...(canonicalSessionId
					? [
							join(
		...
		let rejected: WorkerTeamStateRootResolution | null = null;
		for (const stateRoot of authorityRoots) {
			...
			if (!rejected || rejected.reason === "missing_or_invalid_identity")
				rejected = resolved;
		}
		return rejected
...
export async function resolveWorkerNotifyTeamStateRoot(
	...
	if (resolvedAuthority.context) {
		...
		const explicitTeamRoot =
			typeof env.OMX_TEAM_STATE_ROOT === "string"
				...
				: "";
		const canonicalSessionId =
			resolvedAuthority.context.session_binding?.canonical_session_id;
		const authorityRoots = [
			...new Set([
				...(explicitTeamRoot ? [resolve(cwd, explicitTeamRoot)] : []),
				...(canonicalSessionId
	...
	if (metadataRoot && resolve(cwd, metadataRoot) === direct.stateRoot)
		return direct;
	if (metadataRoot) {
		const resolved = await validateWithSource(
