...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- When it reports `role_routing_unavailable`, do not fabricate `agent_type`; before Ralplan-originated planning, state, HUD, runtime, or delegation work, run `omx ralplan preflight --json` and stop on `unsupported_documented_leader_proof`. Never fake the role via a prompt label or infer authority from session/thread/pointer/transcript/cwd state.',
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_policy: 'When the native surface exposes agent_type role routing, each parallel Codex dispatch must set agent_type to an installed OMX role and never omit it for OMX work. When it reports role_routing_unavailable, do not fabricate agent_type; before Ralplan-originated planning, state, HUD, runtime, or delegation work, run omx ralplan preflight --json and stop on unsupported_documented_leader_proof. Never fake the role via a prompt label or inferred identity. Phase completion must wait for active native subagent threads to finish.',
