...
const LEADER_PANE_SAME_CLASSIFIED_STATE_SUPPRESSED_REASON =
...
export function setLeaderNudgeTestHooksForTests(hooks = {}) {
	leaderNudgeTestHooks.beforeLeaderAttentionRename =
		typeof hooks.beforeLeaderAttentionRename === "function"
			? hooks.beforeLeaderAttentionRename
			: null;
	leaderNudgeTestHooks.afterLeaderAttentionRename =
		typeof hooks.afterLeaderAttentionRename === "function"
			? hooks.afterLeaderAttentionRename
			: null;
	leaderNudgeTestHooks.beforeGlobalNudgeStateRename =
		typeof hooks.beforeGlobalNudgeStateRename === "function"
			? hooks.beforeGlobalNudgeStateRename
			: null;
	leaderNudgeTestHooks.afterGlobalNudgeStateRename =
		typeof hooks.afterGlobalNudgeStateRename === "function"
...
function removeTeamFromLeaderNudgeState(state, teamName) {
	...
	if (
		state?.last_nudged_by_team &&
		typeof state.last_nudged_by_team === "object"
	) {
		delete state.last_nudged_by_team[teamName];
	}
	if (
		state?.last_idle_nudged_by_team &&
		typeof state.last_idle_nudged_by_team === "object"
	) {
		delete state.last_idle_nudged_by_team[teamName];
	}
}

async function teamStateAllowsLeaderNudge(teamDir) {
	...
	const currentPhase = safeString(
		phase?.current_phase || phase?.phase || "",
	).trim();
...
export function resolveLeaderNudgeIntervalMs() {
	...
	if (parsed !== null && parsed >= 10_000 && parsed <= 30 * 60_000)
		return parsed;
	return 30_000;
}
...
export function resolveLeaderAllIdleNudgeCooldownMs() {
	const raw = safeString(
		process.env.OMX_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS || "",
	);
	const parsed = asNumber(raw);
	// Default: 30 seconds.
...
function buildLeaderActionGuidance(
	teamName,
	{
	...
	const inProgress = Number.isFinite(taskCounts.in_progress)
		? taskCounts.in_progress
		: 0;
	const pendingFollowUpTasks =
		allWorkersIdle && pending > 0 && blocked === 0 && inProgress === 0;
...
export async function checkWorkerPanesAlive(tmuxTarget, workerPaneIds = []) {
	...
	try {
		const result = await runProcess(
			"tmux",
			["list-panes", "-t", sessionName, "-F", "#{pane_id} #{pane_pid}"],
			2000,
		);
		...
		const workerPaneIdSet = new Set(
			Array.isArray(workerPaneIds)
				? workerPaneIds
						.map((paneId) => safeString(paneId).trim())
						.filter(Boolean)
				: [],
		);
		const relevantLines =
			workerPaneIdSet.size > 0
				? lines.filter((line) =>
						workerPaneIdSet.has(line.split(/\s+/, 1)[0] || ""),
					)
				: lines;
...
export async function isLeaderStale(stateDir, thresholdMs, nowMs) {
	const scopedDirs = await getScopedStateDirsForCurrentSession(stateDir).catch(
		() => [],
	);
	for (const scopedDir of [...new Set([...scopedDirs, stateDir])]) {
		if (!existsSync(join(scopedDir, "hud-state.json"))) continue;
		return isLeaderRuntimeStale(scopedDir, thresholdMs, nowMs);
	}
...
async function resolveCurrentSessionId(stateDir) {
	...
	return normalizeValidSessionId(
		(await readUsableSessionState(resolve(stateDir, "..", "..")))?.session_id,
	);
}
...
async function readWorkerStatusSnapshot(stateDir, teamName, workerName) {
	...
	try {
		if (!existsSync(path))
			return { state: "unknown", current_task_id: "", missing: true };
		const parsed = JSON.parse(await readFile(path, "utf-8"));
		return {
			state:
				safeString(parsed && parsed.state ? parsed.state : "unknown") ||
				"unknown",
			current_task_id: safeString(
				parsed && parsed.current_task_id ? parsed.current_task_id : "",
			).trim(),
			missing: false,
		};
	} catch {
...
async function readWorkerStatusState(stateDir, teamName, workerName) {
	const snapshot = await readWorkerStatusSnapshot(
		stateDir,
		teamName,
		workerName,
	);
...
async function readWorkerHeartbeatSnapshot(stateDir, teamName, workerName) {
	...
	try {
		...
		const parsed = JSON.parse(await readFile(path, "utf-8"));
		return {
			turn_count: Number.isFinite(parsed?.turn_count)
				? parsed.turn_count
				: null,
			missing: false,
		};
	...
	try {
		...
		for (const entry of taskFiles) {
			try {
				...
				const id = safeString(
					parsed?.id || entry.replace(/^task-/, "").replace(/\.json$/, ""),
				).trim();
				const status =
...
async function readTeamProgressSnapshot(stateDir, teamName, workerNames) {
	...
	const missingSignalWorkers = workerSnapshot.filter(
		({ status_missing, heartbeat_missing }) =>
			status_missing || heartbeat_missing,
	).length;
...
function readPreviousWorkerTurnCounts(previousSignature) {
	try {
		...
		return new Map(
			workers
				.map((worker) => [
					safeString(worker?.worker).trim(),
					Number.isFinite(worker?.turn_count)
						? Number(worker.turn_count)
						: null,
				])
...
function hasWorkerTurnProgress(workerSnapshot, previousTurnCounts) {
	return workerSnapshot.some((worker) => {
		...
		if (!Number.isFinite(worker.turn_count) || worker.turn_count === null)
			return false;
		const previousTurnCount = previousTurnCounts.get(worker.worker);
		return (
			Number.isFinite(previousTurnCount) &&
			previousTurnCount !== null &&
			worker.turn_count > previousTurnCount
		);
	});
}
...
function normalizeMailboxMessages(rawMailbox) {
	...
	if (
		rawMailbox &&
		typeof rawMailbox === "object" &&
		Array.isArray(rawMailbox.messages)
	) {
...
export async function emitTeamNudgeEvent(
	teamDir,
	teamName,
	reason,
	orchestrationIntent,
...
export async function maybeNudgeTeamLeader({
	...
	const nowIso = new Date().toISOString();
	const scopedStateDirs = await getScopedStateDirsForCurrentSession(
		stateDir,
	).catch(() => []);
	...
	if (
		!nudgeState.last_nudged_by_team ||
		typeof nudgeState.last_nudged_by_team !== "object"
	) {
		nudgeState.last_nudged_by_team = {};
	}
	if (
		!nudgeState.last_idle_nudged_by_team ||
		typeof nudgeState.last_idle_nudged_by_team !== "object"
	) {
		nudgeState.last_idle_nudged_by_team = {};
	}
	if (
		!nudgeState.progress_by_team ||
		typeof nudgeState.progress_by_team !== "object"
	) {
		nudgeState.progress_by_team = {};
	}

	const candidateTeamNames = new Set();
	const candidateTeamDirs = new Map();
	const currentSessionId = await resolveCurrentSessionId(stateDir);
	const deepInterviewActive = currentSessionId
		? await isDeepInterviewStateActive(stateDir, currentSessionId).catch(
				() => false,
			)
	...
	const candidateStateDirs = [...new Set([...scopedStateDirs, stateDir])];
	try {
		for (const scopedDir of candidateStateDirs) {
			...
			const phaseSnapshot = await readTeamPhaseSnapshot(
				scopedDir,
				teamName,
				nowIso,
			);
			...
			if (parsed.active === true) {
				...
				candidateTeamDirs.set(teamName, join(scopedDir, "team", teamName));
			}
		}
	} catch {
		// Non-critical
	...
	for (const candidateStateDir of candidateStateDirs) {
		const canonicalFallbackTeams = await listNotifyCanonicalActiveTeams(
			cwd,
			currentSessionId,
			candidateStateDir,
		).catch(() => []);
		for (const team of canonicalFallbackTeams) {
			...
			if (!teamName) continue;
			candidateTeamNames.add(teamName);
			candidateTeamDirs.set(teamName, dirname(team.path));
		}
	}

	// Use pre-computed staleness (captured before HUD state was updated this turn)
	const leaderStale =
		typeof preComputedLeaderStale === "boolean"
			? preComputedLeaderStale
			: false;

	for (const teamName of candidateTeamNames) {
		const teamDir =
			candidateTeamDirs.get(teamName) || join(stateDir, "team", teamName);
		const teamStateRoot = dirname(dirname(teamDir));
		if (!(await teamStateAllowsLeaderNudge(teamDir))) {
		...
		try {
			...
			if (existsSync(srcPath)) {
				...
				tmuxSession = safeString(
					raw && raw.tmux_session ? raw.tmux_session : "",
				).trim();
				leaderPaneId = safeString(
					raw && raw.leader_pane_id ? raw.leader_pane_id : "",
				).trim();
				ownerSessionId = safeString(
					raw && raw.leader && raw.leader.session_id
		...
		let mailbox = null;
		try {
			const mailboxPath = join(teamDir, "mailbox", "leader-fixed.json");
		...
		const workerNames = Array.isArray(workers)
			? workers
					.map((w) => safeString(w && w.name ? w.name : ""))
					.filter(Boolean)
			: [];
		const workerPaneIds = Array.isArray(workers)
			? workers
					.map((w) => safeString(w && w.pane_id ? w.pane_id : ""))
					.filter(Boolean)
			: [];
		...
		if (canonicalLeaderPaneId) {
			...
			if (resolvedLeaderTarget?.paneTarget) {
				...
				["target_is_hud_pane", "pane_cwd_mismatch"].includes(
					safeString(resolvedLeaderTarget.reason).trim(),
				)
			) {
				tmuxTarget = "";
			}
			...
			workerNames.length > 0
				? await Promise.all(
						workerNames.map((workerName) =>
							readWorkerStatusState(teamStateRoot, teamName, workerName),
						),
					)
				: [];
		...
		const progressSnapshot = await readTeamProgressSnapshot(
			teamStateRoot,
			teamName,
			workerNames,
		);
		const prevProgress =
			...
			typeof nudgeState.progress_by_team[teamName] === "object"
				? nudgeState.progress_by_team[teamName]
				: {};
		const previousSignature = safeString(prevProgress.signature || "");
		const previousProgressAtIso = safeString(
			prevProgress.last_progress_at || "",
		);
		const previousProgressAtMs = previousProgressAtIso
			? Date.parse(previousProgressAtIso)
			: NaN;
		const previousTurnCounts = readPreviousWorkerTurnCounts(previousSignature);
		const workerTurnProgress = hasWorkerTurnProgress(
			progressSnapshot.workerSnapshot,
			previousTurnCounts,
		);
		const progressChanged =
			!previousSignature ||
			previousSignature !== progressSnapshot.signature ||
			workerTurnProgress;
		const extraProgressEvidenceMs = await readLatestTeamProgressEvidenceMs(
			cwd,
			teamName,
			teamStateRoot,
		).catch(() => Number.NaN);
		const effectiveProgressAtMs =
			progressChanged || !Number.isFinite(previousProgressAtMs)
				? nowMs
		...
		const prev =
			...
			typeof nudgeState.last_nudged_by_team[teamName] === "object"
				? nudgeState.last_nudged_by_team[teamName]
				: {};
		...
		const dueByTime =
			!Number.isFinite(prevAtMs) || nowMs - prevAtMs >= intervalMs;
		const ackWithoutStartEvidence = hasNewMessage
			? await getAckWithoutStartEvidence(teamStateRoot, teamName, newest)
			: null;

		const prevIdle =
			nudgeState.last_idle_nudged_by_team[teamName] &&
			typeof nudgeState.last_idle_nudged_by_team[teamName] === "object"
				? nudgeState.last_idle_nudged_by_team[teamName]
				: {};
		const prevIdleAtIso = safeString(prevIdle.at || "");
		const prevIdleAtMs = prevIdleAtIso ? Date.parse(prevIdleAtIso) : NaN;
		const dueByIdleCooldown =
			!Number.isFinite(prevIdleAtMs) || nowMs - prevIdleAtMs >= idleCooldownMs;
		...
		const stalePanesNudge =
			paneStatus.alive && leaderStale && !hasFreshProgressEvidence;
		const staleFollowupDue = stalePanesNudge && dueByTime;
		const hasActionableNewMessage =
			hasNewMessage && (allowFreshMailboxNudges || leaderStale);

		let nudgeReason = "";
		let text = "";
		if (shouldSendAllIdleNudge) {
			nudgeReason =
				leaderActionState === "done_waiting_on_leader"
			...
			text =
				...
				buildWorkerStartEvidenceReminder(
					teamName,
					ackWithoutStartEvidence.worker,
				);
		...
		const unreadLeaderMessageCount = messages.filter(
		...
		const persistLeaderNudgeBookkeeping = async ({
			...
			const leaderAttentionPath = join(teamDir, "leader-attention.json");
			try {
				await atomicWriteJsonNoParentCreate(
					...
					leaderAttention,
					{
						beforeRename: async (tempPath) => {
							...
							if (!(await teamStateAllowsLeaderNudge(teamDir))) {
								throw new Error(
									"team_state_gone_or_shutdown_before_leader_attention_rename",
								);
							}
						},
						afterRename: async () => {
							if (leaderNudgeTestHooks.afterLeaderAttentionRename) {
								await leaderNudgeTestHooks.afterLeaderAttentionRename({
									stateDir: teamStateRoot,
									teamName,
									path: leaderAttentionPath,
								});
							}
						},
					},
			...
			try {
				await atomicWriteJsonNoParentCreate(nudgeStatePath, nextNudgeState, {
					beforeRename: async (tempPath) => {
						...
						if (!(await teamStateAllowsLeaderNudge(teamDir))) {
							throw new Error(
								"team_state_gone_or_shutdown_before_nudge_state_rename",
							);
						}
					},
					afterRename: async () => {
						if (leaderNudgeTestHooks.afterGlobalNudgeStateRename) {
							await leaderNudgeTestHooks.afterGlobalNudgeStateRename({
								stateDir: teamStateRoot,
								teamName,
								path: nudgeStatePath,
							});
		...
		const orchestrationIntent = resolveLeaderNudgeIntent({
			nudgeReason,
			leaderActionState,
		});
		if (!(await teamStateAllowsLeaderNudge(teamDir))) {
		...
		if (!tmuxTarget) {
			...
			if (
				!(await persistLeaderNudgeBookkeeping({
					orchestrationIntent,
					recordLastNudged: true,
				}))
			) {
				continue;
			}
			await emitLeaderNudgeDeferredEvent(
				teamDir,
				teamName,
		...
		if (!paneGuard.ok) {
			const deferredReason =
				paneGuard.reason === "pane_running_shell"
		...
		if (paneAlreadyShowsVisibleLeaderState(paneGuard.paneCapture, capped)) {
			...
			try {
				await logTmuxHookEvent(logsDir, {
					...
					suppression_reason:
						LEADER_PANE_SAME_CLASSIFIED_STATE_SUPPRESSED_REASON,
				});
			} catch {
				/* ignore */
			}
