...
**Adapted role-intent step:** On a `role_routing_unavailable` surface, before each App Architect or Critic spawn, run `omx ralplan role-intent write --role <role> --parent-thread <leader-thread-id>`, then spawn; the recorded validated intent is the authoritative role, never a prompt label.
